...
class MoveMultipleLayersDigitizingMode(QgsMapToolEmitPoint):
    
    MESSAGE_HEADER = "Move from multiple layers"
    ...
    def next(self):
        if self.step == 0:
            ...
            if self.checkSelection(self.doMove):
                self.doMove()
   
    def checkSelection(self, func):
        totalLayers = 0
        ...
        if totalLayers == layersWithNoSelection:      
            ...
            self.messageBarUtils.showYesCancel(self.MESSAGE_HEADER, "%d out of %d vector layers have no selection. Proceed on the other layers?" % (layersWithNoSelection, totalLayers), 
                                               QgsMessageBar.WARNING, func, self.deactivate)
            return False
        return True
            ...
            for layer in layers:
                ...
                if layer.type() == QgsMapLayer.VectorLayer and layer.selectedFeatureCount() > 0:
                    ...
                    if layer.isEditable():
                        ...
                        layer.beginEditCommand("Move from multiple layers")
                    else:
                    ...
                    for f in features:
                        ...
                        layer.changeGeometry(f.id(), outGeom)
                        current += 1
    ...
    def applyPrevious(self):    
        if self.checkSelection(partial(self.doMove, True)):
            self.doMove(True)
